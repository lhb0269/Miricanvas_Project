...
| **Expected Result** | - URL이 변경되지 않음 (`https://www.miricanvas.com/ko` 유지)<br>- 에러 메시지 표시 (예: "이메일 또는 비밀번호가 올바르지 않습니다")<br>- 로그인 다이얼로그가 닫히지 않음<br>- 입력 필드가 활성 상태로 유지 |
...
| **Expected Result** | - URL이 변경되지 않음<br>- 에러 메시지 표시 (예: "이메일 또는 비밀번호가 올바르지 않습니다")<br>- 로그인 다이얼로그가 닫히지 않음<br>- 비밀번호 필드가 초기화되거나 유지됨 |
...
| **Precondition** | - 로그인 다이얼로그 열림<br>- 이메일 로그인 폼 표시 상태 |
| **Test Step** | 1. 이메일 입력 필드를 빈 상태로 유지<br>2. 비밀번호 입력 필드에 `gksqlc9784!` 입력<br>3. "로그인" 버튼 클릭 시도 |
| **Expected Result** | - 로그인 버튼이 비활성화 상태이거나<br>- 클릭 시 유효성 검사 에러 메시지 표시 (예: "이메일을 입력해주세요")<br>- URL이 변경되지 않음<br>- 로그인 시도가 진행되지 않음 |
...
| **Precondition** | - 로그인 다이얼로그 열림<br>- 이메일 로그인 폼 표시 상태 |
| **Test Step** | 1. 이메일 입력 필드를 빈 상태로 유지<br>2. 비밀번호 입력 필드를 빈 상태로 유지<br>3. "로그인" 버튼 클릭 시도 |
| **Expected Result** | - 로그인 버튼이 비활성화 상태이거나<br>- 클릭 시 유효성 검사 에러 메시지 표시<br>- URL이 변경되지 않음<br>- 로그인 시도가 진행되지 않음 |
