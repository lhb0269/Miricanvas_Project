/**
 * TC-EMAIL-007: 잘못된 비밀번호로 로그인 실패
 *
 * @description
 * 미리캔버스에서 유효한 이메일과 잘못된 비밀번호를 사용하여 로그인 실패를 검증하는 테스트
 *
 * @testCase
 * - Precondition: 로그인 다이얼로그 열림, 이메일 로그인 폼 표시 상태
 * - Action: 유효한 이메일과 잘못된 비밀번호 입력 후 로그인 시도
...
const TEST_CONFIG = {
  ...
  // 테스트용 계정 정보 (유효한 이메일 + 잘못된 비밀번호)
  credentials: {
    validEmail: 'lhb0269@naver.com',
...
const SELECTORS = {
  ...
  loginDialog: {
    ...
    errorMessage: "span[data-f='Span-ba96']:has-text('잘못된 비밀번호입니다')",
    // 비밀번호 입력 필드의 부모 컨테이너
...
`;
  ...
  if (result.status === 'FAIL') {
    ...
    report += `- 비밀번호 검증이 적절히 작동하는지 확인\n`;
    report += `- 다양한 잘못된 비밀번호 패턴에 대한 추가 테스트 고려\n`;
  }
...
async function runTest(): Promise<TestResult> {
  ...
  try {
    log('='.repeat(80));
    log('테스트 시작: TC-EMAIL-007 - 잘못된 비밀번호로 로그인 실패');
    log('='.repeat(80));
    ...
    // ========================================================================
    // ACT (실행): 유효한 이메일 + 잘못된 비밀번호로 로그인 시도
    // ========================================================================

    log('ACT: 유효한 이메일 + 잘못된 비밀번호로 로그인 시도');

    // Step 1: 유효한 이메일 입력
    const stepResult1: StepResult = {
      stepNumber: 1,
      description: `이메일 입력 필드에 유효한 이메일 ${TEST_CONFIG.credentials.validEmail} 입력`,
      status: 'PASS',
      timestamp: new Date()
    };

    try {
      log('Step 1: 유효한 이메일 입력 시도');
      await page.waitForSelector(SELECTORS.loginDialog.emailInput, { timeout: TEST_CONFIG.timeouts.elementVisible });
      await page.fill(SELECTORS.loginDialog.emailInput, TEST_CONFIG.credentials.validEmail);

      testResult.steps.push(stepResult1);
      log(`✓ Step 1 완료: 유효한 이메일 입력됨 (${TEST_CONFIG.credentials.validEmail})`, 'SUCCESS');
    } catch (error) {
    ...
    const stepResult2: StepResult = {
      stepNumber: 2,
      description: `비밀번호 입력 필드에 잘못된 비밀번호 ${TEST_CONFIG.credentials.invalidPassword} 입력`,
      status: 'PASS',
      timestamp: new Date()
    };

    try {
      log('Step 2: 잘못된 비밀번호 입력 시도');
      await page.waitForSelector(SELECTORS.loginDialog.passwordInput, { timeout: TEST_CONFIG.timeouts.elementVisible });
      await page.fill(SELECTORS.loginDialog.passwordInput, TEST_CONFIG.credentials.invalidPassword);

      testResult.steps.push(stepResult2);
      log('✓ Step 2 완료: 잘못된 비밀번호 입력됨', 'SUCCESS');
    } catch (error) {
    ...
    // 로그인 시도 전 스크린샷 캡처
    const beforeLoginScreenshot = `${TEST_CONFIG.screenshotDir}\\before-invalid-password-login-${getTimestamp()}.png`;
    ...
    const assertResult1: StepResult = {
      stepNumber: 5,
      description: 'URL이 변경되지 않음 (로그인 실패 확인)',
      status: 'PASS',
      timestamp: new Date()
    };
    ...
    const assertResult2: StepResult = {
      stepNumber: 6,
      description: '에러 메시지 표시 확인 ("잘못된 비밀번호입니다")',
      status: 'PASS',
      timestamp: new Date()
    };
    ...
    try {
      ...
      // 에러 메시지 요소 대기 (span[data-f='Span-ba96'] 내 "잘못된 비밀번호입니다" 텍스트)
      const errorElement = await page.waitForSelector(SELECTORS.loginDialog.errorMessage, {
        timeout: TEST_CONFIG.timeouts.errorMessageVisible,
        state: 'visible'
      });

      if (!errorElement) {
        throw new Error('에러 메시지가 표시되지 않았습니다.');
      }
      ...
      // 비밀번호 입력 컨테이너 클래스 변경 확인 (sc-267d8ce6-0 ejTrKt -> sc-267d8ce6-0 gGTyzN)
      ...
      if (!passwordContainerClass || !passwordContainerClass.includes('gGTyzN')) {
        throw new Error(`비밀번호 컨테이너 클래스가 에러 상태로 변경되지 않았습니다. 실제: "${passwordContainerClass}"`);
      }

      log('✓ 비밀번호 입력 컨테이너 클래스 변경 확인됨 (sc-267d8ce6-0 gGTyzN)');
      ...
      try {
        const allSpans = await page.$$eval('span', spans =>
          spans.map(span => ({
            dataF: span.getAttribute('data-f'),
            class: span.className,
            text: span.textContent
          })).filter(s => s.text?.includes('잘못된') || s.text?.includes('비밀번호') || s.dataF?.includes('Span'))
        );
        log(`발견된 span 요소: ${JSON.stringify(allSpans, null, 2)}`);
      } catch (debugError) {
    ...
    // Assert 4: 비밀번호 필드가 초기화되거나 유지되는지 확인
    const assertResult4: StepResult = {
      stepNumber: 8,
      description: '비밀번호 필드가 초기화되거나 유지됨',
      status: 'PASS',
      timestamp: new Date()
    };

    try {
      log('Assert 4: 비밀번호 필드 상태 확인');
      ...
      log('✓ Assert 4 통과: 비밀번호 필드가 활성 상태로 유지됨', 'SUCCESS');
    } catch (error) {
    ...
    // 로그인 실패 후 스크린샷 캡처
    const afterLoginFailScreenshot = `${TEST_CONFIG.screenshotDir}\\after-invalid-password-fail-${getTimestamp()}.png`;
...
(async () => {
  try {
    ...
    const reportPath = `${TEST_CONFIG.reportDir}\\TC-EMAIL-007-Report-${getTimestamp()}.md`;
