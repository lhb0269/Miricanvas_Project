/**
 * TC-EMAIL-001: 유효한 계정 정보로 이메일 로그인 성공
 *
 * @description
 * 미리캔버스에서 유효한 이메일과 비밀번호를 사용하여 로그인 성공을 검증하는 테스트
 *
 * @testCase
 * - Precondition: 로그아웃 상태, 유효한 계정 정보 보유
 * - Action: 이메일/비밀번호 입력 후 로그인
 * - Expected: 워크스페이스 페이지로 이동 및 필수 UI 요소 표시
 *
...
`;
  ...
  if (result.status === 'FAIL') {
    ...
    report += `- 테스트 실행 시간이 적절한지 검토\n`;
    report += `- 추가 엣지 케이스 테스트 고려\n`;
  }
...
async function runTest(): Promise<TestResult> {
  const testResult: TestResult = {
    testId: 'TC-EMAIL-001',
    testName: '유효한 계정 정보로 이메일 로그인 성공',
    status: 'PASS',
    startTime: new Date(),
    endTime: new Date(),
    duration: 0,
  ...
  try {
    log('='.repeat(80));
    log('테스트 시작: TC-EMAIL-001 - 유효한 계정 정보로 이메일 로그인 성공');
    log('='.repeat(80));
    ...
    // 로그인 전 스크린샷 캡처
    const beforeLoginScreenshot = `${TEST_CONFIG.screenshotDir}\\before-login-${getTimestamp()}.png`;
    ...
    // Assert 3: 로그인 다이얼로그 자동 닫힘 확인
    const assertResult3: StepResult = {
      stepNumber: 8,
      description: '로그인 다이얼로그가 자동으로 닫혔는지 확인',
      status: 'PASS',
      timestamp: new Date()
    };

    try {
      log('Assert 3: 로그인 다이얼로그 닫힘 확인');
      ...
      log('✓ Assert 3 통과: 로그인 다이얼로그가 닫힘', 'SUCCESS');
    } catch (error) {
    ...
    log('='.repeat(80));
    log(`테스트 완료: 모든 검증 통과 (로그인 경로: ${loginPath})`, 'SUCCESS');
    log('='.repeat(80));
...
(async () => {
  try {
    ...
    const reportPath = `${TEST_CONFIG.reportDir}\\TC-EMAIL-001-Report-${getTimestamp()}.md`;
