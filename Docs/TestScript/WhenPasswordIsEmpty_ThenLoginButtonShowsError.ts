/**
 * Test Case: TC-EMAIL-009
 * Test Name: WhenPasswordIsEmpty_ThenLoginButtonShowsError
 * Description: 비밀번호 미입력 시 로그인 불가
 *
 * Test Steps:
 * 1. 미리캔버스 홈페이지 접속
 * 2. 로그인 다이얼로그 열기
 * 3. 이메일 필드에 유효한 이메일 입력
 * 4. 비밀번호 필드를 비워둠 (미입력)
 * 5. 로그인 버튼 클릭
 * 6. "공백 없이 입력해주세요." 에러 메시지 확인
 * 7. 에러 컨테이너 클래스 변경 확인 (.sc-267d8ce6-0.ejTrKt → .sc-267d8ce6-0.gGTyzN)
 *
 * Expected Result:
 * - 에러 메시지 표시: span[data-f='Span-ba96'] 내 "공백 없이 입력해주세요." 텍스트
...
const TEST_CONFIG = {
  ...
  testData: {
    email: "lhb0269@naver.com", // 유효한 이메일
    password: "", // 빈 비밀번호
  },
  screenshots: {
    enabled: true,
...
async function takeScreenshot(
  ...
  const filename = `TC-EMAIL-009-${name}-${timestamp}.png`;
...
async function runTest(): Promise<void> {
  ...
  try {
    // ========== Arrange: 테스트 준비 ==========
    log("=== TC-EMAIL-009: 비밀번호 미입력 시 로그인 불가 테스트 시작 ===");
    ...
    await takeScreenshot(page, "05-email-entered-password-empty");
    ...
    // ========== Assert: 결과 검증 ==========
    log("에러 메시지 확인 중...", {
      selector: SELECTORS.loginDialog.errorMessage,
    ...
    if (!errorText || !errorText.includes("공백 없이 입력해주세요")) {
      throw new Error(
